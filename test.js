                    ...
                    notifications.forEach(n => {
                        if (!n.is_read) {
                            ...
                            if (!lastSeenNotificationIds.has(n.id)) {
                                if (!window.isFirstNotificationPoll && !recentToastMessages.has(n.message)) {
        ...
        async function apiCall(url, options = {}) {
            ...
            if (isGet && window.apiPrefetchCache && window.apiPrefetchCache[url]) {
        ...
        async function loadMasterPreview() {
            ...
            try {
                ...
                const allCols = data.columns.slice();
                const allColSet = new Set(allCols);
                headEl.innerHTML = '<tr>' + allCols.map(col =>
                ...
                if (data.data && data.data.length > 0) {
                    bodyEl.innerHTML = data.data.map(row => {
                        ...
                        const cells = allCols.map(col => {
                            ...
                            return `<td class="px-3 py-2 text-sm text-gray-900 whitespace-nowrap">${escapeHtml(v)}</td>`;
                        }).join('');
        ...
        async function loadRulePageData() {
            ...
            try {
                updateProgress('Loading files and rules in parallel...');
                
                // Start network requests concurrently for maximum speed
                const filesPromise = getAllFilesList();
                const p2Promise = loadSavedPhase2();
                const p3Promise = loadSavedPhase3();
                const p4Promise = loadSavedPhase4();
                ...
                // Phase 1 must be awaited after dropdown population, but concurrent with P2/P3/P4
                await loadSavedPhase1();
                
                // Await completion of the remaining phases
                await Promise.all([p2Promise, p3Promise, p4Promise]);
                ...
                [1, 2, 3, 4].forEach(phase => {
                    apiCall(`/api/rules/${phase}?validation_id=${vId}`).then(data => {
                        if (phase === 1 && data && data.success && data.rules && data.rules.length > 0) {
                            ...
                            if (config.file_id) {
                                let sUrl = config.file_id.startsWith('master_') ? `/api/master/${config.file_id.replace('master_', '')}/sheets` : (config.file_id.startsWith('primary_') ? null : `/api/files/${config.file_id}/sheets`);
                                if (sUrl) apiCall(sUrl);
                                if (config.sheet_name) {
                                    let cUrl = config.file_id.startsWith('master_') ? `/api/master/${config.file_id.replace('master_', '')}/columns` : (config.file_id.startsWith('primary_') ? null : `/api/files/${config.file_id}/columns/${encodeURIComponent(config.sheet_name)}`);
                                    if (cUrl) apiCall(cUrl);
                                }
                            }
                            if (config.primary_file) {
                                apiCall(`/api/primary/preview/${encodeURIComponent(config.primary_file)}`);
                            }
                        }
                    }).catch(() => {});
                });
                ...
                if (rulesData.success && rulesData.rules && rulesData.rules.length > 0) {
                    ...
                    if (isVal1or3) {
                        if (p1Config.file_id) {
                            ...
                            const masterColsData = await apiCall(`/api/master/${cleanFileId}/columns`);
                            if (masterColsData.success && masterColsData.columns) {
                                masterColsData.columns.forEach((c, idx) => {
        ...
        async function processAllRules(force = false) {
            try {
                ...
                if (vid !== 1) {
                    selectedSourceFiles = getSelectedSourceFiles();
        ...
        async function loadFinalProcessedHistory() {
            ...
            try {
                const data = await apiCall(`/api/processed/files?validation_id=${vid}`);
                if (data.success) {
                    // Save to cache for instant render on next tab switch
                    if (data.files) {
        ...
        function addIfConditionRow(triggerBtn, savedCond = null) {
            ...
            row.innerHTML = `
                <div class="flex flex-wrap items-center gap-1.5">
                    ...
                    <input type="text" class="if-val flex-1 min-w-[100px] text-xs border border-gray-300 rounded px-2 py-1" placeholder="Value">
                    <select class="if-col-value hidden flex-1 min-w-[120px] text-xs border border-gray-300 rounded px-2 py-1">${colOpts}</select>
            ...
            if (savedCond) {
                ...
                if (colSel && savedCond.column && Array.from(colSel.options).some(o => o.value === savedCond.column)) {
                    colSel.value = savedCond.column;
                }
                if (savedCond.value_type === 'column' && savedCond.value) {
                    const ccSel = row.querySelector('.if-col-value');
                    if (ccSel && Array.from(ccSel.options).some(o => o.value === savedCond.value)) {
            ...
            // If this is a position aggregator, also set the literal value
            if (isPos && v !== '') {
        ...
        function onIfCondChange(el) {
            ...
            if (agg === 'blank' || agg === 'not_blank') {
                if (vtype) vtype.disabled = true;
                if (valInput) { valInput.disabled = true; valInput.placeholder = '(no value needed)'; valInput.value = ''; }
                if (colSelect) { colSelect.disabled = true; colSelect.value = ''; }
            } else {
                if (vtype) vtype.disabled = false;
                if (vtype && vtype.value === 'column') {
                    if (valInput) { valInput.classList.add('hidden'); valInput.disabled = true; valInput.value = ''; }
                    if (colSelect) {
                        colSelect.classList.remove('hidden');
                        colSelect.disabled = false;
                        colSelect.innerHTML = buildIfColumnOptions();
                    ...
                    if (colSelect) { colSelect.classList.add('hidden'); colSelect.disabled = true; colSelect.value = ''; }
                    if (valInput) {
                        valInput.classList.remove('hidden');
                        valInput.disabled = false;
        ...
        async function applyFormula() {
            ...
            if (type === 'IF_CONDITION') {
                ...
                showFormulaProgress('Applying IF formula, please wait...');
                try {
                    const formData = new FormData();
                    formData.append('formula_type', 'IF_CONDITION');
                    formData.append('source_columns', '');
                    ...
                    if (data.success) {
                        ...
                        showToast(`IF formula applied to ${(data.rows_affected || 0).toLocaleString()} rows in '${data.column_name}'${actIdPart}`, 'success', 4500);
                        closeFormulaModal();
                        await Promise.all([
                            loadMasterPreview(),
                            loadMasterStats(),
                        ...
                        showToast(data.message || data.detail || 'Apply failed', 'error');
                    }
                } catch (error) {
                    hideFormulaProgress();
                    document.getElementById('formula-apply-btn').disabled = false;
            ...
            if (type === 'SUMIF' || type === 'COUNTIF' || type === 'VLOOKUP' || type === 'HLOOKUP') {
                ...
                try {
                    ...
                    document.getElementById('formula-apply-btn').disabled = false;
                    if (data.success) {
                        showToast(`Formula applied to ${(data.rows_affected || 0).toLocaleString()} rows in '${data.column_name}' · Activity #${data.activity_id || '?'} saved`, 'success', 4500);
                        closeFormulaModal();
                        await Promise.all([
                            loadMasterPreview(),
                            loadMasterStats(),
        ...
        async function deleteMasterColumn(columnName) {
            ...
            try {
                ...
                if (data.success) {
                    const actIdPart = data.activity_id ? ` \u00b7 Activity #${data.activity_id} saved` : '';
                    showToast(`Column '${columnName}' deleted${actIdPart}`, 'success', 4000);
                    await Promise.all([
                        loadMasterPreview(),
                        loadMasterStats(),
                        loadMasterSummary(),
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (!dryRun && data.total_rows_affected > 0) {
                        const actIdPart = data.activity_id ? ` \u00b7 Activity #${data.activity_id} saved` : '';
                        showToast(`Replaced ${data.total_rows_affected} occurrences${actIdPart}`, 'success', 4500);
            ...
            function attachListeners() {
                try {
                    ...
                    const ok2 = wire('btn-formula', function() {
                        ...
                        // Verify the modal element exists BEFORE opening so we surface any issues
                        const modalEl = document.getElementById('formula-modal');
                        console.log('[Reconciliation] formula-modal element:', modalEl ? 'FOUND' : 'NOT FOUND');

                        // CRITICAL: Apply inline styles to guarantee visibility regardless of Tailwind state
                        function forceShowFormulaModal() {
                            if (!modalEl) return;
                            // Remove hidden class
                            modalEl.classList.remove('hidden');
                            // Apply inline styles that override any CSS issues
                            ...
                            modalEl.style.justifyContent = 'center';
                            modalEl.style.backgroundColor = 'rgba(0,0,0,0.5)';
                            console.log('[Reconciliation] FORCE-SHOWED formula-modal with inline styles');
                        }

                        if (typeof openFormulaModal === 'function') {
                            try {
                                openFormulaModal();
                                forceShowFormulaModal();
                                if (typeof showToast === 'function') {
                                    showToast('Formula modal opened', 'success');
                                }
                            } catch (e) {
                                console.error('[Reconciliation] openFormulaModal threw:', e);
                                forceShowFormulaModal();
                                if (typeof showToast === 'function') {
                                    showToast('Opened Formula (fallback): ' + e.message, 'warning');
                                }
                            }
                        } else {
                            // Function not defined - force-show the modal anyway
                            console.warn('[Reconciliation] openFormulaModal not defined, force-showing modal');
                            forceShowFormulaModal();
                            if (typeof showToast === 'function') {
                                showToast('Formula modal opened (no function)', 'info');
                            }
                        }
                    }, 'Formula');
...
async function val13LoadAndSave() {
    ...
    try {
        const colsData = await apiCall(`/api/master/${folderId}/columns`);
        
        if (colsData.success) {
            val13CurrentColumns = colsData.columns;
...
async function val13LoadFileHeadersSilent(folderId) {
    try {
        ...
        const colsData = await apiCall(`/api/master/${cleanFolderId}/columns`);
        if (colsData.success) {
            val13CurrentColumns = colsData.columns;
            window.GLOBAL_PHASE1_COLUMNS = val13CurrentColumns;
            val13RenderColumns();
